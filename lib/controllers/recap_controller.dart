...
class RecapData {
  ...
  /// Period budget = dailyBudget * days for every range (month = dailyBudget *
  /// daysInMonth). Single budget basis shared with the Today ring and the
  /// Budgets screen, so editing the daily budget moves every period.
  final double budget;
...
RecapData buildRecapData(
  ...
  // retained for source-compat; the monthly budget is now derived from
  // goals.dailyBudget, so this seed-frozen envelope-cap sum is ignored.
  ...
  // single budget basis for every range: dailyBudget * days (month =
  // dailyBudget * daysInMonth). the seed-frozen envelope-cap sum no longer
  // backs the monthly target, so editing the daily budget moves every period.
  final budget = goals.dailyBudget * days;
  var spent = 0.0;
  var moveKcal = 0;
