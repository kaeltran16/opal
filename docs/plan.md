...
**DONE:** U01–U24, U21b, U25, U26, U27-code (HealthKit/Notifications services live, iOS-gated), **Wave 6 (Handoff #2 client screens)**.  **REMAINING (all device/Mac/account-gated, none buildable on Windows):** U24 background scheduler + APNs push; live-inbox IMAP verification; device-only QA for U25 expanded/rest state + U26 Siri voice; U27 HealthKit real-data (paid-account-gated); plus Handoff #2 device QA (true SF Symbols/haptics/sheet physics) + a real `/v1/routine` backend endpoint for the Routine Generator (mock today).
