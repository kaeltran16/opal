...
**Backend U22–U23 integrated → `master`** (`a8ea938`, `9629aac`): OpenRouter-backed Pal proxy in `server/` (`/chat`,`/review`,`/parse` + device register/token auth, deepseek default model) + `HttpPalService` client swapped in behind `PalService` via `--dart-define=PAL_BASE_URL` (mock stays the default). **U24 (email IMAP worker) still unbuilt** — `server/` has no email worker and there is no `RealEmailSyncService`; only `MockEmailSyncService` exists.
...
**DONE:** U01–U23, U21b, U25, U26, U27-code (HealthKit/Notifications services live, iOS-gated).  **REMAINING:** **U24** (email IMAP worker + `RealEmailSyncService` — buildable here); device-only QA for U25 expanded/rest state + U26 Siri voice; U27 HealthKit real-data (paid-account-gated).
