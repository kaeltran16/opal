...
**Backend U22–U23 integrated → `master`** (`a8ea938`, `9629aac`): OpenRouter-backed Pal proxy in `server/` (`/chat`,`/review`,`/parse` + device register/token auth, deepseek default model) + `HttpPalService` client swapped in behind `PalService` via `--dart-define=PAL_BASE_URL` (mock stays the default).

**Backend U24 (email IMAP worker) integrated → `main`** (uncommitted — awaiting approval). **Pull model** (locked with user): the client holds the IMAP app-password in the device keychain and sends it per sync; the server stores **no credentials and no email state**. Server (`server/src/`): `imap.ts` (`MailboxClient` seam + `ImapFlowClient` via `imapflow`+`mailparser`), `receipts.ts` (deterministic `filterBySender` + LLM `extractReceipt`), `email.ts` (`EmailWorker`), two bearer-guarded routes `POST /v1/email/{test,sync}`. IMAP-auth failure on `/sync` → **422** (not 401, so it never collides with the bearer-token-retry path). Client: `RealEmailSyncService` (gated on `PAL_BASE_URL` exactly like `palService`; mock stays default), persists the account (minus password) in `shared_preferences` so `isConnected` survives restart, stages the `SyncStatus` sequence around the single round-trip. Dedup is **client-side by `sourceRef`** (`EntryRepository.existsBySourceRef`); `EmailDashboardController.syncNow` now materialises imports as money `Entry`s (`source: email`) — the previously-missing "push entries back" step, so synced receipts finally reach Today/Spending. Server: 39 tests green, build clean. Flutter: 105 tests green, analyze clean. **Deferred (device/Mac-gated, like U27 HealthKit):** the 15-min background scheduler + APNs push (would need server-side encrypted cred storage + a scheduler + a device to verify); and live IMAP against a real inbox (no test account — the worker is verified via a faked `MailboxClient`).

**Native U25–U27 integrated → `ios-native-u25-u27` `457dd9b`** (Mac session; builds for simulator, 96 tests green, analyze clean). All staged Swift/Dart from `7a0a8e6`/`a03387c` is now wired into the Xcode project and call-sites; see `docs/ios-native-setup.md`.
- **Xcode project** done programmatically via `ios/configure_native_targets.rb` (idempotent `xcodeproj` script — the CLI can't do target/membership surgery by hand safely): 4 Runner-target Swift files added to compile sources; new **`OpalWidgets`** app-extension target (Live Activity + `@main` `OpalWidgetsBundle` + shared `OpalWorkoutAttributes`), embedded in Runner. The **Embed App Extensions** phase is ordered **before** Flutter's "Thin Binary" phase to break the `ExtractAppIntentsMetadata` ↔ embed-appex build cycle.
- **Native glue:** `AppDelegate.didInitializeImplicitFlutterEngine` registers both bridges on the engine messenger (via a borrowed `registrar(forPlugin:)` — Flutter 3.44 implicit-engine API); `SceneDelegate` overrides `willConnectTo`/`openURLContexts`, calls `super` (preserves plugin URL forwarding), and forwards `opal://` URLs to `OpalIntentsBridge` (covers Live-Activity taps + AppIntent opens).
- **Dart call-sites:** `workout_session_controller` starts/updates/ends the Live Activity (updates only on content change — the island self-ticks from `startedAt`); `app.dart` routes `SiriShortcutsService.deepLinks` → `router.go` with an immediate-duplicate guard + `donateShortcuts`/`consumeInitialDeepLink` on first frame. New `test/deep_link_routing_test.dart` (route + dedup).
- **U25 VERIFIED on the iPhone 17 Pro simulator:** booted into an active session → the workout Live Activity rendered in the Dynamic Island with the live-ticking elapsed timer. Remaining manual/device QA: island expanded view + rest-countdown state (need a tap), and U26 "Hey Siri" voice (needs a device — Spotlight/deep-link plumbing is verified).
- **U27 HealthKit entitlement deliberately NOT added:** a free Apple Personal Team (no paid account) can't provision `com.apple.developer.healthkit`, and adding it breaks device signing. `HealthKitService` degrades gracefully without it (`a03387c`); real HealthKit data + ritual-reminder notification *scheduling* call-sites stay unverified until a paid account exists.

**Wave 6 — Handoff #2 (`Downloads/expensepal (2)`) integrated → `main`** (uncommitted at build time; analyze clean, full suite green). The second design handoff's new/changed surface, built orchestrator-owns-shared + 8 fan-out agents on disjoint feature files (no worktrees, per user rule).
- **Rituals reframed (full model replacement, locked with user):** flat `Ritual` → `RitualRoutine` + ordered `RitualStep` (three time-of-day routines: Morning/Midday/Evening). Step *completion stays an `Entry`* (`Entry.ritualId == step.id`) — the single source of truth, so Today/Monthly/Profile ring math is **untouched**. Drift `schemaVersion` 1→2 (drop `rituals`; create `ritual_routines`/`ritual_steps` + `bills`/`subscriptions`/`pal_notes`); seeder marker `v2→v3`. `RitualRepository` rewritten (routines+steps, `upsertRoutine`/`deleteRoutine`/`reorderRoutines`); `rituals_controller` now exposes per-routine step progress + `toggleStep`/`completeStep`. Onboarding seeds the 3 default routines. `Cadence` enum removed (only `Ritual` used it).
- **New models/enums/repos:** `Bill`, `Subscription`, `PalNote`; `RitualTone`, `NoteKind`; `BillRepository`/`SubscriptionRepository`/`PalNoteRepository` (+ seed data + providers). `PalService.generateRoutine(goal, available)` seam added (Mock + Http `/v1/routine`).
- **Pal composer is now the single input surface:** the center FAB opens the unified composer (compact → chat, seedable via `/pal-composer?seed=`), **replacing** the old Quick-Actions menu and the standalone Ask Pal. (Quick-actions overlay retained but no longer FAB-wired.)
- **10 screens (8 parallel agents, disjoint files):** Pal Composer (03/04) · Rituals tab **timeline** layout + guided **Routine Player** (13/13b) + Builder rewrite · Evening Close-Out (14, always-dark purple, wired to live routine steps) · Streak Celebration (15) · Weekly Review (17) · Subscriptions (18) · Bills (23) · Weekly Plan (24) · Pal Inbox (25) · AI Routine Generator. Reflective screens deep-link-seed the composer (Close-Out, Weekly Review, Pal Inbox).
- **Entry points + routes (orchestrator-owned `router.dart`/`providers.dart`/`app_icon.dart`/codegen):** FAB→composer; Profile→Subscriptions/Bills/Weekly review; Start-Workout→"Generate with Pal"; Move→Weekly plan. 12 SF-Symbol substitutes added for preview.
- **Dispatch lesson reinforced:** never run `flutter test` concurrently — overlapping suites contend on the shared `.dart_tool` and produce spurious failures (a profile_test "fail" that passes in isolation). Run one suite at a time.

**Wave 7 — Design reconciliation (Handoff #4, `Downloads/expensepal (4)`) integrated → `main`** (uncommitted — awaiting approval; **109 tests green**, full suite ~11s, no hangs). Driven by the audit in `design-audit/DESIGN_DIFFERENCES.md` (Handoff #4 JSX/spec vs the on-disk Flutter app). Built orchestrator-owns-shared + 8 fan-out agents on disjoint screen files (no worktrees, per user rule); orchestrator owned `router.dart` + the Bills/Subs removal + central verification.
- **Renames confirmed complete (locked with user — keep):** user-facing **Move→Workout** and **Rituals→Routines** across tab bar, ring stats, summary tiles, nav titles, inbox chips, profile stats, Pal/review copy. Internal identifiers left unchanged (color tokens `move`/`rituals`, `EntryType.move`, `RitualTone`, route paths `/move`·`/rituals`, DB names, `.g.dart`).
- **You tab redesigned** to `YouTabScreen`: profile card + inset sections Goals / Reviews / Integrations / Data / Account; new **`BudgetSheet`** (`lib/widgets/budget_sheet.dart`, Daily/Weekly segmented rescale) wired to the Daily-budget Goals row; old README-era stat grid dropped.
- **HealthKit / calorie UI removed** from the Workout hero (kcal/HR card) and the Today timeline (badge now minutes, not `kcal`); matches the Handoff-#4 "no HealthKit" directive. (Native `HealthKitService` stays as iOS-gated dead code per U27.)
- **Workout tab hero** redesigned (workouts-vs-goal headline, 56px mini ring, 7-day strip, Volume/Time/Records); **Email dashboard** rebuilt (stats tiles, "Pal noticed" card + Review CTA, sync settings, Gmail glyph); Routines tab/player polish; Pal/Reviews/Onboarding copy aligned.
- **Bills & Subscriptions REMOVED (product decision, locked with user):** `bills_screen.dart`/`subscriptions_screen.dart` deleted, their routes + the You-tab "Money" section stripped, zero dangling refs. The data layer (`Bill`/`Subscription` models, repos, `money_recurring_controller`, seed) now survives as **dormant/unreachable code** — candidate for later removal.
- **Pre-existing onboarding bug fixed:** the "45 min" workout-goal chip was untappable (lower half clipped outside the scroll viewport, so `dailyMoveMinutes` never left the 60 default); reclaimed 12px of step spacing so the chip is hittable.
- **Test-hang fixed (was dispatch-lesson #4, now applied to boot tests):** the 6 full-`LoopApp` + real-DB boot tests tripped flutter_test's `!timersPending` invariant on the Drift `StreamQueryStore.markAsClosed` zero-duration timer (confirmed pre-existing at `HEAD`). Added shared `test/support/flush_provider_timers.dart` (`pumpWidget(SizedBox)` + `pump(10ms)`) and called it as the last line of those tests. Also updated 5 tests that asserted the old UI to the redesigned UI (no test weakened).

**DONE:** U01–U24, U21b, U25, U26, U27-code (HealthKit/Notifications services live, iOS-gated), **Wave 6 (Handoff #2 client screens)**, **Wave 7 (Handoff #4 design reconciliation; Bills/Subs removed)**.  **REMAINING (all device/Mac/account-gated, none buildable on Windows):** U24 background scheduler + APNs push; live-inbox IMAP verification; device-only QA for U25 expanded/rest state + U26 Siri voice; U27 HealthKit real-data (paid-account-gated); plus Handoff #2/#4 device QA (true SF Symbols/haptics/sheet physics) + a real `/v1/routine` backend endpoint for the Routine Generator (mock today). **Audit long-tail (Windows-now, not yet done):** assorted `[STYLE]`/`[LAYOUT]` nits and data-model gaps flagged in `design-audit/DESIGN_DIFFERENCES.md` (e.g. routine `estMin`/`lastDone`, cardio distance/pace, some SF-symbol map entries); remove the dormant Bills/Subs data layer.
...
**Git:** baseline `4ee45cd` → foundation `18146dc` → wave-1 `ee21c46` → wave-2 `c68f13b` → waves 3+4 → wave-5 `0f3d719` → U21 polish `78714f9` → backend U22/U23 `a8ea938`,`9629aac` → rename-to-Opal `0c75f2d` → native staging `7a0a8e6`,`a03387c` → native integration `457dd9b` (branch `ios-native-u25-u27`). Default branch is now `main`. The stale `unit/u10-move-tab` worktree at `f096b29` is unused (worktrees abandoned per user rule).
